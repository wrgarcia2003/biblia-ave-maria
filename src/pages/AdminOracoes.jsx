...
import * as oracoesService from '../services/oracoesService'
...
const AdminOracoes = ({ onVoltar }) => {
  ...
  const carregarLista = async () => {
    ...
    setErro('')
    try {
      const r = await oracoesService.listarOracoes({})
      setLista(r || [])
    } catch (e) {
      setErro('Erro ao carregar orações')
    } finally {
      setCarregando(false)
    }
  }
  ...
  const carregarSegmentos = async (id) => {
    const segs = await oracoesService.listarSegmentos(id)
    setSegmentos(segs || [])
  }
  ...
  const handleUploadAudio = async (e, item) => {
    ...
    const url = `${process.env.REACT_APP_SUPABASE_URL}/storage/v1/object/audios-oracoes/${name}`
    ...
    const publicUrl = `${process.env.REACT_APP_SUPABASE_URL}/storage/v1/object/public/audios-oracoes/${name}`
    const audio = new Audio()
    audio.src = URL.createObjectURL(file)
    await new Promise((resolve) => { audio.onloadedmetadata = resolve })
    const dur = Math.round(audio.duration || 0)
    const patch = await fetch(`${process.env.REACT_APP_SUPABASE_URL}/rest/v1/oracoes?id=eq.${item.id}`, {
      method: 'PATCH',
  ...
  const handleImportarTexto = async (e, item) => {
    ...
    const segs = linhas.map((l, i) => ({ oracao_id: item.id, numero: i + 1, texto: l.trim(), tempo_inicio: null, tempo_fim: null }))
    await oracoesService.deletarSegmentos(item.id)
    await oracoesService.inserirSegmentos(segs)
  ...
  const calcularTimestamps = async (item, metodo = 'uniforme') => {
    ...
    for (const t of tempos) { await oracoesService.atualizarTimestamp(t.id, Number(t.inicio.toFixed(2)), Number(t.fim.toFixed(2))) }
    await carregarSegmentos(item.id)
  }
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-brand-50 to-neutral-50">
      <div className="max-w-4xl mx-auto p-6 pb-32 md:pb-6">
        ...
        <h1 className="text-2xl font-bold text-brand-900 mb-6">Gerenciar Orações</h1>
        {erro && (
