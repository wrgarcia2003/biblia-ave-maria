...
const OracoesList = ({ onVoltar, onOpenDetail }) => {
  ...
  const carregar = async () => {
    ...
    try {
      const [cats, list] = await Promise.all([
        oracoesService.listarCategorias(),
        oracoesService.listarOracoes({ categoria: categoria || null, q }),
      ])
      setCategorias(cats || [])
      setItens(list || [])
    } catch (e) {
      ...
      <div className="max-w-3xl mx-auto p-6">
        <div className="flex items-center justify-between mb-6">
          <div className="flex items-center gap-2">
            ...
            <h1 className="text-2xl font-bold text-brand-900">Orações</h1>
          </div>
        ...
        {carregando ? (
          ...
          <div className="text-center text-brand-700 bg-brand-50 border border-brand-100 rounded-lg p-6">Nenhuma oração encontrada</div>
        ) : (
