...
import { ArrowLeft, Heart, Play, Pause, SkipBack, SkipForward } from 'lucide-react'
import * as oracoesService from '../services/oracoesService'

const renderTexto = (texto) => {
  if (!texto) return null
  return texto.split('\n').map((l, i) => (
    <p key={i} className="mb-3 text-lg leading-relaxed text-neutral-800">{l}</p>
  ))
}

const OracaoDetail = ({ item, onVoltar }) => {
  ...
  useEffect(() => { if (!item?.id) return; (async () => { const s = await oracoesService.listarSegmentos(item.id); setSegmentos(s || []) })() }, [item?.id])
    ...
    <div className="min-h-screen bg-gradient-to-br from-brand-50 to-neutral-50">
      <div className="max-w-3xl mx-auto p-6">
        ...
        <div className="text-sm text-neutral-600 mb-4">
          {item.categoria} {item.fonte ? `• ${item.fonte}` : ''}
        </div>
        <div className="bg-white rounded-xl shadow-sm border border-neutral-100 p-6">
          ...
          <div ref={containerRef} className="max-h-96 overflow-y-auto">
            ...
            )) : renderTexto(item.texto_md || item.texto || ''))}
          </div>
        </div>
      </div>
    </div>
